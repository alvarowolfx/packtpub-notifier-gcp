...
class PackPubCrawler {
  fetchPageBody() {
    return fetch(urlFreeLearning).then(res => {
      return res.text();
    });
  }
  ...
  scrapeCurrentBook(documentSelector) {
    ...
    if (titleElements.length > 0) {
      ...
      if (descriptionElements.length > 0) {
        ...
        //console.log(descEl);
        description = documentSelector(descEl)
          .text()
          .trim();
      }
    }
  ...
  scrapeOldBooks(documentSelector) {
    let oldBooks = [];
    let oldBookElements = documentSelector(
      '.section-outer.grey .section-inner > a'
    );
  ...
  fetchBooksFromPacktPub() {
    return this.fetchPageBody().then(body => {
